...
- 不建设与正式能力无关的通用 Mock 服务；普通 CRUD、登录、短信和对象存储样板不进入仓库；
- 网络模拟必须服务某项已准入能力，并覆盖断流、取消、重试、竞态等真实边界；
- 优先使用能力就近的测试 fixture；只有多个正式能力确实共享服务进程时，才允许新增独立开发服务；
- 开发辅助代码同样必须有明确消费者、类型检查、自动化测试和同步文档，不能成为低价值代码的避难所。
